...
# ╔═╡ 1b4fa766-1a61-4488-a21b-838a7daab301
function my_sum(v)
	sum = 0
	for i in v
		sum += i
...
# ╔═╡ ed905b80-2d15-4654-b34d-c5930ef941b3


# ╔═╡ 9071eb6d-0217-4d8c-b65c-2d6676e4a34a
colored_line(example_vector)
...
@bind l Slider(0:7, show_value=true, default=40)
...
# ╔═╡ 54d9e1ed-3fec-47ce-af99-133a8a2d6229
...
function gaussian_kernel_1D(n; σ = 1)
	len = 2*n + 1
	kernel = zeros(len)
	gₓ(x) = (1/sqrt(2*π*σ^2)) * exp(-(x^2)/(2*σ^2))
	for (idx, j) in enumerate(-n:n)
		kernel[idx] = gₓ(j)
	end
	return kernel
end
...
# ╔═╡ e5a42fa2-cfa2-409b-ab1b-8ebb140988cc
...
# ╔═╡ bad9185f-1980-44ac-9e1e-65025083cdfd
...
# ╔═╡ 4c5f666a-67d0-4d02-928a-97c5e83b1e5b
colored_line(box_blur(example_vector, l))
...
function convolve(v::AbstractVector, k)
	new_v = zeros(length(v))
	l = Int((length(k)-1)/2) # 2l + 1 is the lenght of the vector k
	_sum = 0
	
	for i in 1:length(v)
        for (idx,j) in enumerate((-l+i):(l+i))
...
# ╔═╡ 0c5f5692-1134-4394-90cd-204e4f966062
...
function convolve(M::AbstractMatrix, K::AbstractMatrix)
	...
	for i = 1:rm, j = 1:cm
		portion = [extend(M, p, q) for p = i-lr:i+lr, q = j-lc:j+lc]
		new_M[i, j] = sum(portion .* K) 
	end
	return new_M
...
# ╔═╡ 5bb8779f-c51b-4286-9f4d-3b53324d2085
...
# ╔═╡ 3fd6bd6c-7430-4b09-b209-5ac2c5c81afd
begin 
end

# ╔═╡ cca2e015-4b44-4b89-9f0a-3be69ef6bd6a
...
# ╔═╡ 86982fae-c76b-4945-9563-601d177083c9


# ╔═╡ d4b3b8b7-caf0-48d5-b93e-728212198085
function sq(p)
	return RGB(p.r^2, p.g^2, p.b^2)
end

# ╔═╡ fead91bd-ff87-4709-8678-f1d4ffbab86a
function sq_root(p)
	return RGB(sqrt(p.r),sqrt(p.g),sqrt(p.b))
end
...
function with_sobel_edge_detect(image)
	...
	G(img1, img2) = sq_root.(sq.(img1) + sq.(img2))
	
    return G(Gx, Gy)
	return image
end

# ╔═╡ f2191ca0-8f8f-49c7-b777-608b797dd0f8
...
# ╔═╡ cba0e4e7-f5eb-4b53-b8a4-003d23accab2
#=╠═╡
